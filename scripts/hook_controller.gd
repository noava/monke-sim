@icon("res://assets/sprites/hook.png")
class_name HookController
extends Node

@export_group("Controls")

@export var launch_action_name: String = "l_mouse"## Input Map action name that triggers hook's launch; default left mouse button

@export var retract_action_name: String = "r_mouse"## Input Map action name that triggers hook's retraction; default left mouse button
...
func _physics_process(delta: float) -> void:
	...
	# Toggle Grapple TODO: Add in settings?
...
func _launch_hook() -> void:
	...
	hook_target_node.global_position = hook_raycast.get_collision_point()#for some reason this was originally hook_target_node.position = raycast.get_collision_point() - body.global_position, even though that was a less precise and more convoluted way of doing it
